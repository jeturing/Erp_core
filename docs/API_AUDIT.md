...
1. **Billing comparison MoM** → Implementado en backend pero frontend no consume
   - **Solución:** Conectar `Billing.svelte` a `billingApi.getComparison()`

2. **Audit logs** → Backend `GET /api/logs/audit` existe pero frontend no lo usa aún
   - **Solución:** Crear página de Audit Log o integrar en Settings

3. **Tenant status handlers** → Suspend/Reactivate implementados pero sin feedback visual
   - **Solución:** Agregar toast notifications y polling de estado

### Low (Nice-to-have)

1. **Proxy/cache de Odoo** → No hay endpoint para verificar estado de instancia Odoo después de provisión

...
function Partners($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!----> Nuevo Partner</button></div> <div class="grid grid-cols-1 sm:grid-cols-3 gap-4"><div class="stat-card"><div class="text-xs font-semibold uppercase text-gray-500 mb-1">Activos</div> <div class="text-2xl font-bold text-success dark:text-emerald-400">${escape_html(summary.active)}</div></div> <div class="stat-card"><div class="text-xs font-semibold uppercase text-gray-500 mb-1">Pendientes</div> <div class="text-2xl font-bold text-warning dark:text-amber-400">${escape_html(summary.pending)}</div></div> <div class="stat-card"><div class="text-xs font-semibold uppercase text-gray-500 mb-1">Total Leads</div> <div class="text-2xl font-bold text-primary dark:text-blue-400">${escape_html(summary.total_leads)}</div></div></div> <div class="flex flex-col sm:flex-row gap-3"><div class="relative flex-1">`);
    Search($$renderer2, {
      size: 16,

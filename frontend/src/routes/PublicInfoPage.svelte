<script lang="ts">
  ...
  import { ArrowRight, CheckCircle2, ShieldCheck } from 'lucide-svelte';

  export let slug: 'about' | 'privacy' | 'terms' | 'data-processing' | 'security' | 'sla';
  ...
  const pages: Record<string, PageContent> = {
    ...
    privacy: {
      ...
      intro:
        'SAJET procesa datos de cuenta, operación, facturación y soporte para prestar el servicio, proteger la plataforma y cumplir obligaciones legales y contractuales.',
      sections: [
        {
    ...
    terms: {
      ...
      intro:
        'El uso de SAJET requiere credenciales válidas, cumplimiento de políticas de seguridad, uso legítimo de la plataforma y pago oportuno de los servicios contratados.',
      sections: [
        {
          title: 'Uso aceptable',
          paragraphs: [
    ...
    'data-processing': {
      ...
      intro:
        'SAJET actúa como procesador u operador técnico sobre datos necesarios para ejecutar el servicio contratado y mantener la plataforma disponible y segura.',
      sections: [
        {
          title: 'Alcance',
          paragraphs: [
    ...
    security: {
      ...
      intro:
        'SAJET opera con controles de autenticación, aislamiento lógico por tenant, auditoría, validación de parámetros y hardening de la superficie web y API.',
      sections: [
        {
    ...
    sla: {
      ...
      intro:
        'Los niveles de servicio varían por plan, criticidad y acuerdo comercial, pero SAJET opera con objetivos de continuidad, monitoreo y respuesta sobre la plataforma central.',
      sections: [
        {
          title: 'Cobertura',
          paragraphs: [
...
<div class="min-h-screen bg-white font-inter">
  ...
  <section class="bg-gradient-hero pt-32 pb-16">
    <div class="max-w-4xl mx-auto px-6 text-center">
      <span class="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/15 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-white/80">
        {page.eyebrow}
      </span>
      <h1 class="mt-6 text-4xl md:text-5xl font-jakarta font-extrabold text-white leading-tight">
        {page.title}
      </h1>
      <p class="mt-5 text-base md:text-lg text-white/80 max-w-2xl mx-auto leading-relaxed">
        {page.intro}
      </p>
    </div>
  </section>
    ...
    <div class="max-w-4xl mx-auto px-6">
      <div class="grid gap-6">
        {#each page.sections as section}
          <article class="rounded-card-lg border border-border bg-white p-6 shadow-soft">
            <div class="flex items-center gap-3 mb-4">
              <div class="w-10 h-10 rounded-full bg-primary-light flex items-center justify-center">
                <ShieldCheck class="w-5 h-5 text-primary" />
              </div>
              <h2 class="text-xl font-jakarta font-bold text-slate-dark">{section.title}</h2>
            </div>
            <div class="space-y-3">
              {#each section.paragraphs as paragraph}
                <p class="text-sm md:text-[15px] text-slate leading-relaxed">{paragraph}</p>
              {/each}
            </div>
          </article>
        {/each}
      ...
      <div class="mt-8 rounded-card-lg border border-primary/15 bg-white p-6 shadow-soft">
        <div class="flex items-start gap-3">
          <CheckCircle2 class="w-5 h-5 text-primary mt-0.5 flex-shrink-0" />
          <div>
            <p class="text-sm font-semibold text-slate-dark">Documento público resumido</p>
            <p class="mt-2 text-sm text-slate leading-relaxed">
              Este contenido resume la operación y criterios públicos vigentes de SAJET. Para contratos, anexos o versiones
              formales adaptadas a un cliente, canaliza la solicitud por ventas o soporte.
            </p>
          </div>
        </div>
        ...
        <div class="mt-5 flex flex-col sm:flex-row gap-3">
          ...
          <a
            href={formalRequestHref}
            class="inline-flex items-center justify-center gap-2 border border-primary text-primary hover:bg-primary hover:text-white font-jakarta font-semibold text-sm px-6 py-3 rounded-btn transition-all"
          >
            Solicitar versión formal
          </a>
        </div>
      </div>
    </div>

...
class IrActionsActions(models.Model):
    ...
    def set_virtual_data(self, record):
        action_virtual = self.env['ir.actions.center'].search([('action_id', '=', self.id)], limit=1)
        ...
        record['name'] = self['display_name'] = action_virtual.name
        if len(views_order):
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        for item in res:
            if item.get('id', False) and item.get('views', False):
                self.browse(item['id']).set_virtual_data(item)
        return res
...
class ActionView(models.Model):
    ...
    view_mode = fields.Selection(selection_add=[('dashboard', 'Dashboard')], ondelete={'dashboard': 'cascade'})
...
class IrActionsReport(models.Model):
    ...
    def copy_report(self):
        ...
        studio_view = self.env['studio.template.center'].search(
            [('view_id', '=', template_view.id), ('key', '=', self.id), ('is_current', '=', True)], limit=1)
        if len(studio_view):
            studio_view.copy({'view_id': new_template.id, 'undo': False, 'redo': False, 'key': new.id})
        new.write({
...
class IRModelData(models.Model):
    ...
    @api.model
    def xmlid_to_res_ids(self, xmlid=[]):
        groups_info = {}
        for x in xmlid:
            if x.find("!") == -1:
                group = self.env.ref(x)
                groups_info[x] = {'id': group.id, 'display_name': group.display_name}
        return groups_info

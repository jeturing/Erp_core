...
from odoo.addons.test_new_api.tests.test_new_fields import update
...
class UsMessengerProject(models.Model):
    ...
    eval_context_description = fields.Text(compute="_compute_eval_context_description", readonly=True)
    common_developer_code = fields.Text(string="Common Developer Code")
    ...
    use_custom_code = fields.Boolean(string="Use Custom Code", default=False, help="If you have changed the standard "
                                                                                   "code to suit your needs, "
                                                                                   "you can always switch the bot's "
                                                                                   "execution between your code and "
                                                                                   "the standard valid one")
    ...
    def _get_eval_context(self, job, log):
        ...
        start_time = time.time()
        code = self.common_code if self.use_custom_code else self.common_developer_code
        safe_eval(
            (code or "").strip(), eval_context, mode="exec", nocopy=True
        )
    ...
    def extract_fields(self, root, model_name, record):
        ...
        for field in data:
            ...
            field_value = None
            if field_name == "common_developer_code" and model_name == "us.messenger.project":
                self.update({"common_developer_code": field.text})
    ...
    def find_task(self, root, task_ref, ):
        for task_record in root.findall(".//record[@model='us.messenger.task']"):
            if task_record.get("id") == task_ref:
                ...
                code = task_record.find(".//field[@name='developer_code']").text
    ...
    def action_switch_developer_code(self):
        self.use_custom_code = False
    ...
    def write(self, vals):
        ...
        if 'operator_ids' in vals:
            ...
            partners = [self.env['res.users'].browse(record).partner_id.id for record in operators_to_remove]
            channels_ids = [int(l.ref2) for l in links]
            bot_channels = self.env['discuss.channel'].search([('id', 'in', channels_ids), ('messenger_operator_id.id', 'in', partners)])

            if operators_to_remove:
                for channel in bot_channels:
                    chanel_part_ids = set(channel.channel_partner_ids.ids)
                    result = chanel_part_ids - set(partners)
                    channel.write({'channel_partner_ids': list(result), 'messenger_operator_id': None})
            # if operators_to_add:
    ...
    def _unlink_partner(self):
        for record in self:
            ...
            # messenger_automation = self.env['us.messenger.trigger.automation'].search(
            #     [('project_id', '=', self.id)])
            # for r in messenger_automation.automation_id:
            #     r.unlink()
            #
            # messenger_server_action = self.env['us.messenger.trigger.webhook'].search(
            #     [('project_id', '=', self.id)])
            # for r in messenger_server_action.action_server_id:
            #     r.unlink()

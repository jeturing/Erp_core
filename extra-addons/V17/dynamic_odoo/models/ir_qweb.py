...
class IrQWeb(models.AbstractModel):
    ...
    def load_template(self, view_id):
        element = self._get_template(view_id)[0]
        template_result = {'arch': etree.tostring(element)}
    ...
    def _get_converted_image_data_uri(self, base64_source):
        if type(base64_source) == bool:
            return False
        return super(IrQWeb, self)._get_converted_image_data_uri(base64_source)

    def _prepare_environment(self, values):
        irQweb = super()._prepare_environment(values)
        if self.env.context.get("Studio", False) and hasattr(irQweb, "_prepare_frontend_environment"):

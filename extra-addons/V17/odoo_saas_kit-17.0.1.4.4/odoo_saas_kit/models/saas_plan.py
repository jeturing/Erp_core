...
class SaasPlans(models.Model):
    ...
    saas_base_url = fields.Char(string="SaaS Domain(Base URL)", required=True, tracking = True,help="Base url of the server on which the Plan container is runing")
    ...
    trial_period = fields.Integer(string="Complimentary(Free) days", default=0, tracking = True,help="No. of free/trial days")
    ...
    state = fields.Selection(
        selection=STATE, string="Status", default="draft", tracking=True)
    contract_count = fields.Integer(
        string='Contract Count', compute='_get_contract_count', readonly=True,help="Count of contracts created from this saas plan")
    ...
    is_all_installed = fields.Boolean(string="All Modules Installed", default=False,help="Automatically check if all the related saas modules are installed")
    ...
    def write(self, vals):
        ...
        if vals.get('saas_module_ids'):
            ...
            uninstalled_modules = self.modules_status_ids.filtered(lambda m: m.status =='uninstalled' and m.module_id.id in self.saas_module_ids.ids)
            self.is_all_installed = False if len(uninstalled_modules) else True
        return res

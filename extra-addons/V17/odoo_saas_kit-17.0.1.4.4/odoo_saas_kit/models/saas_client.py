...
class SaasClient(models.Model):
    ...
    container_port = fields.Char(string="Port",help="PORT on which client's container is runing")
    container_lport = fields.Char(string="Long Conn. Port",help="PORT on which client's container Longpolling/Websocket Connection is runing")
    container_path = fields.Char(string="Path",help="Path of the client's container")
    ...
    last_rebuild_status = fields.Boolean("Last Rebuild Status")
    rebuild_cmd = fields.Char("Container Rebuild Command")
    ...
    def add_custom_domain(self, domain=None, ssl_flag=False):
        """
            Method to generate SSL certificates and nginx vhost conf file for 
            the custom domain used in the SaaS contracts during the SaaS client creation.
        """
        ...
        response = generate_ssl_custom_domain.main_add_domain(subdomain.lower(), domain.lower(), ssl_flag, module_path)
